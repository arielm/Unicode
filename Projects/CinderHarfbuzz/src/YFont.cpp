...
ftHelper(ftHelper)
{
    ...
    if (descriptor.forceMemoryLoad || !descriptor.source->isFilePath())
    {
        memoryBuffer = descriptor.source->getBuffer();
        error = FT_New_Memory_Face(ftHelper->getLib(), (FT_Byte*)memoryBuffer.getData(), memoryBuffer.getDataSize(), descriptor.faceIndex, &face);
    }
    else
    {
    ...
    App::get()->console() << "LOADING FONT: " << face->family_name << " "  << face->style_name << endl;
